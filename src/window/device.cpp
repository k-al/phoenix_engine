...
bool QueueBatch::test_batched (VkPhysicalDevice device, VkQueueFamilyProperties props, size_t fam_index, size_t number) {
    ...
    for (auto check : this->checks) {
        if (!check(device, props, fam_index))
            return false;
    }
...
bool QueueBatch::test_unbatched (VkPhysicalDevice device, VkQueueFamilyProperties props, size_t fam_index, size_t number) {
    if (this->checks.empty()) {
        std::cerr << "no queues specified!\n";
        return true;
    }
    ...
    for (size_t i = 0; i < this->found.size(); ++i) {
        if (!this->found[i] && this->checks[i](device, props, fam_index)) {
...
bool Device::ini (Device_ini ini) {
    ...
    this->queue_batches = ini.queue_batches;
...
std::vector<VkDeviceQueueCreateInfo> Device::get_queue_infos (float* priority) {
    ...
    for (auto batch : *this->queue_batches) { // get all batches
        ...
        for (uint32_t i = 0; i < device_support_count; ++i) { // show them all queues
            if (batch.test_batched(this->physical, device_support[i], i, queue_count[i])) {
                ++queue_count[i];
                break;
            }
        }
        ...
        for (uint32_t i = 0; i < device_support_count; ++i) { // show them all queues
            if (batch.test_unbatched(this->physical, device_support[i], i, queue_count[i])) {
                ++queue_count[i];
            }
        }
...
int32_t Device::check_queue_support (VkPhysicalDevice device) {
    ...
    for (auto batch : *this->queue_batches) { // get all batches
        for (uint32_t i = 0; i < device_support_count; ++i) { // show them all queues
            batch.test_unbatched(device, device_support[i], i, 0);
        }
        if (!batch.is_complete()) {
            std::cerr << "not all queues supported from a batch with size " << this->queue_batches->size() << std::endl;
            return -1;
        }
        batch.test_reset();
    }
    ...
    for (auto batch : *this->queue_batches) { // get all batches
        for (uint32_t i = 0; i < device_support_count; ++i) { // show them all queues
            if (batch.test_batched(device, device_support[i], i, 0)) {
